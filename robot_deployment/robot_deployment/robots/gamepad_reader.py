...
from robot_deployment.convex_mpc_controller.locomotion_controller import ControllerMode
from robot_deployment.convex_mpc_controller.locomotion_controller import GaitType
...
class Gamepad:
  ...
  def update_command(self, event):
    ...
    if self._lb_pressed and self._rb_pressed:
      if not self._estop_flagged:
        logger.info("EStop Flagged, press LEFT joystick to release.")
  ...
  def flag_estop(self):
    if not self._estop_flagged:
      logger.info("Estop flagged by program.")
      self._estop_flagged = True
      while self._mode != ControllerMode.DOWN:
        self._mode = next(self._mode_generator)
