...
from robot_deployment.convex_mpc_controller import com_velocity_estimator
from robot_deployment.convex_mpc_controller import offset_gait_generator
from robot_deployment.convex_mpc_controller import raibert_swing_leg_controller
from robot_deployment.convex_mpc_controller import torque_stance_leg_controller_mpc
from robot_deployment.convex_mpc_controller.gait_configs import CRAWL_CONFIG, TROT_CONFIG, FLYTROT_CONFIG
...
from robot_deployment.worlds import abstract_world, plane_world
...
class LocomotionController(object):
  ...
  def _setup_robot_and_controllers(self):
    ...
    p.setAdditionalSearchPath(os.path.join(LEGGED_GYM_ROOT_DIR, 'resources'))
  ...
  def _handle_mode_switch(self):
    ...
    if self._desired_mode == ControllerMode.DOWN:
      logger.info("Entering joint damping mode.")
      self._flush_logging()
  ...
  def _update_logging(self, action, qp_sol):
    frame = dict(
        ...
        base_angular_velocity_in_base_frame=self._robot.base_angular_velocity_in_base_frame,
  ...
  def _flush_logging(self):
    ...
    logger.info("Data logged to: {}".format(
  ...
  def _handle_gait_switch(self):
    ...
    if self._desired_gait == GaitType.CRAWL:
      logger.info("Switched to Crawling gait.")
      self._gait_config = CRAWL_CONFIG
    elif self._desired_gait == GaitType.TROT:
      logger.info("Switched  to Trotting gait.")
      self._gait_config = TROT_CONFIG
    else:
      logger.info("Switched to Fly-Trotting gait.")
      self._gait_config = FLYTROT_CONFIG
  ...
  def run(self):
    logger.info("Low level thread started...")
    while True:
      ...
      if self._mode == ControllerMode.DOWN:
        ...
        logger.info("Running loop terminated, exiting...")
        break
