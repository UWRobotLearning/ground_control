...
def adapt(adapt_cfg, env, agent, data):
    ...
    for i in tqdm.tqdm(range(start_i, adapt_cfg.max_steps),
        ...
        if i < adapt_cfg.start_training:
            if adapt_cfg.pretrain_steps > 0:  ## Collect samples from pre-trained policy instead of random
...
def main(cfg: ContinualScriptConfig) -> None:  
    ...
    if not cfg.load_preadapt_buffer:
        env: A1Continual = hydra.utils.instantiate(cfg.preadapt_task)
        agent = pretrained_policy 
        preadapt_data = collect(cfg.preadapt_task, env, agent, cfg.preadapt_collection.dataset_size)
    ...
    env: A1Continual = hydra.utils.instantiate(cfg.preadapt_task_eval_pretrain_obs)
    ...
    env: A1Continual = hydra.utils.instantiate(cfg.pretrain_new_task_eval)
    preadapt_stats = evaluate(cfg.eval, env, pretrained_policy)
    log.info(f"Pre-trained policy in new terrain has stats: {preadapt_stats}")
    env.exit()
    ...
    for i, adapt_task_cfg in enumerate(cfg.adapt_tasks):

        ############################################################ 
        # Adaptation stage:
        #   1. Reset environment with new configuration
        #   2. Set up task (aka reward function)
        #   3. Set up agent (with new observation space)
        #   4. Set up replay buffer and offline dataset
        #   5. Train agent with RLPD in new environment  
        #   6. Collect expert data of this agent in new environment 
        ############################################################

        log.info(f"Adapting to new environment {i+1}!")
        env: A1Continual = hydra.utils.instantiate(adapt_task_cfg)
        agent = SACLearner.create(cfg.adapt_train.seed, env.observation_space,
        ...
        if cfg.save_adapt_agent:
            ...
            checkpoint_manager.save(i, adapt_model, save_kwargs={'save_args': save_args})

        env.exit()

        ############################################################ 
        # Adaptation stage: Expert data collection
        #   1. Collect expert data of this agent in new environment 
        ############################################################

        env: A1Continual = hydra.utils.instantiate(adapt_task_cfg)
        agent = adapt_policy 
        expert_data = collect(adapt_task_cfg, env, agent, cfg.adapt_collection.dataset_size, obs_processor=obs_to_nn_input, action_processor=action_to_env_input) ## Doesn't work yet
        if cfg.save_adapt_buffer:
            if not os.path.exists(cfg.save_adapt_buffer_dir):
                os.makedirs(cfg.save_adapt_buffer_dir)
            save_buffer_filename = os.path.join(cfg.save_adapt_buffer_dir, f"env_{i+1}.pkl")
            with open(save_buffer_filename, 'wb') as f:
                pickle.dump(expert_data, f)
        env.exit()

        datasets.append(expert_data)
        ############################################################ 
        # Adaptation stage: Evaluate policy
        #   1. Evaluate expert policy in original environment
        #   2. Evaluate expert policy in current environment
        ############################################################

        # Evaluate in original environment
        env: A1Continual = hydra.utils.instantiate(cfg.preadapt_task_eval_adapt_obs)
        agent = adapt_policy 
        adapt_og_stats = evaluate(cfg.eval, env, agent, obs_processor=obs_to_nn_input, action_processor=action_to_env_input)
        log.info(f"Expert policy in original environment has stats: {adapt_og_stats}")
        env.exit()

        # Evaluate in current environment
        env: A1Continual = hydra.utils.instantiate(cfg.adapt_tasks_eval[i])
        agent = adapt_policy 
        adapt_current_stats = evaluate(cfg.eval, env, agent, obs_processor=obs_to_nn_input, action_processor=action_to_env_input)
        log.info(f"Expert policy in current environment has stats: {adapt_current_stats}")
        env.exit()


        ############################################################ 
        # Distillation stage:
        #   1. Aggregate all relevant datasets
        #   2. Distill knowledge into agent's networks 
        ############################################################

        # model = adapt_model  ## TODO: Change this to be the distillation model after step 1
        # obs_labels = env.observation_labels
        # distillation_data = get_distillation_data(datasets, int(1e5), obs_labels)
        # distill_policy, distill_model = distill(distillation_data, model)
