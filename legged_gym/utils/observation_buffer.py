...
class ObservationBuffer:
    def __init__(self, num_envs, num_obs, history_steps, device):
        ...
        self.num_obs_total = num_obs * history_steps
    ...
    def reset(self, reset_idxs, new_obs):
        self.obs_buf[reset_idxs] = new_obs.repeat(1, self.history_steps)

    def insert(self, new_obs):
        # Shift observations back.
        self.obs_buf[:, : self.num_obs * (self.history_steps - 1)] = self.obs_buf[:,self.num_obs : self.num_obs * self.history_steps].clone()
    ...
    def get_obs_vec(self, obs_ids):
        ...
        Arguments:
            obs_ids: An array of integers with which to index the desired
                ...
                history_steps - 1 is the oldest observation.
        """

        obs = []
        for obs_id in reversed(sorted(obs_ids)):
            slice_idx = self.history_steps - obs_id - 1
