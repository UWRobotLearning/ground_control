...
class A1Continual(BaseTask):
    def __init__(self, env: EnvConfig, observation: ObservationConfig, terrain: TerrainConfig,
        ...
        self._define_observation_labels()
        self.init_done = True
    ...
    def step(self, actions):
        ...
        if self.control_cfg.clip_setpoint:
            if self.control_cfg.control_type == 'P':
                ...
                ## Added check by Mateo
                try:
                    assert torch.all((self.actions + self.default_dof_pos) >= self.dof_pos_limits[:,0]) and torch.all((self.actions + self.default_dof_pos) <= self.dof_pos_limits[:,1]), "Action setpoints are outside bounds" 
                except:
    ...
    def post_physics_step(self):
        ...
        self._update_camera_location(env_id=0)
    ...
    def _compute_physical_measurements(self):
        ...
        self.base_heights[:] = torch.mean(self.root_states[:,2].unsqueeze(1) - self.measured_heights, dim=1)  ## Assumes legged robot is actor 0
    ...
    def _get_noise_scale_vec(self):
        ...
        for sensor in self.observation_cfg.sensors:
            if sensor == "base_ang_vel":
                ...
                noise_scale_list.append(torch.zeros(self.sensor_dims["base_quat"], device=self.device))
            elif sensor == "base_mass":
                ...
                noise_scale_list.append(torch.zeros(self.sensor_dims["z_pos"], device=self.device))
    ...
    def _define_action_space(self):
        ...
        # action_limit = self.normalization_cfg.clip_actions  ## V0: incorrect
        # lower = self.dof_pos_limits[:,0].detach().cpu().numpy()
        # upper = self.dof_pos_limits[:,1].detach().cpu().numpy() ## V1: more correct, but doesn't seem to work in practice
        assert self.control_cfg.action_scale == 1, "Action scale should be 1 to set action_space properly. Untested if different."
        limits = (self.dof_pos_limits.T - self.default_dof_pos) / self.control_cfg.action_scale
    ...
    def _create_cameras(self):
        ...
        for i in range(1):
            ...
            camera_properties.width = 360*5
            camera_properties.height = 240*5
            ...
            self.camera_handles[i].append(h1)  ## Remove if doesn't work
            self._update_camera_location(env_id=i)

            # ## Robot position:
            # actor_root_state = self.gym.acquire_actor_root_state_tensor(self.sim)
            # root_states = gymtorch.wrap_tensor(actor_root_state)
            # base_pos = (root_states[i, :3] + self.env_origins[i]).cpu().numpy()
            # self.gym.set_camera_location(h1, self.envs[i], gymapi.Vec3(2, -1.5, 0.9), gymapi.Vec3(*base_pos))
            # self.camera_handles[i].append(h1)


            # Attach camera 2 to the first rigid body of the first actor in the environment. The camera offset is relative to the position of the actor, the camera_rotation
    ...
    def _create_envs(self):
        ...
        for i in range(1):
            ...
            # robot_origin = self.root_states[0, :3] + self.env_origins[0]
            # camera_transform = gymapi.Transform()
            # camera_transform.p = gymapi.Vec3(3, -2.25, 0.9) + gymapi.Vec3(*robot_origin)
            # camera_rotation_1 = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), np.deg2rad(125))
            # camera_rotation_2 = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 1, 0), np.deg2rad(30))
            # camera_rotation = (camera_rotation_1 * camera_rotation_2).normalize()
