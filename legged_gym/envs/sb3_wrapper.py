...
# Taken from https://github.com/NVIDIA-Omniverse/orbit/blob/main/source/extensions/omni.isaac.orbit_tasks/omni/isaac/orbit_tasks/utils/wrappers/sb3.py
...
class Sb3VecEnvWrapper(VecEnv):
    """Wraps around A1 environment for Stable Baselines3.
    ...
    In contrast to the A1 environment, stable-baselines expect the following:
    ...
    def __init__(self, env: A1):
        ...
        self.num_envs = self.env.num_envs ## self.unwrapped.num_envs
        self.sim_device = self.env.device ## self.unwrapped.device
        self.render_mode = "human" ## self.unwrapped.render_mode 
        # obtain gym spaces
        # note: stable-baselines3 does not like when we have unbounded action space so
        #   we set it to some high value here. Maybe this is not general but something to think about.
        observation_space = self.unwrapped.single_observation_space["policy"]  ## TODO: Not sure whether this should be the full observation space (a.k.a critic observation space) or just the actor observation space. TODO: make a bounded gym Box out of it
        action_space = self.unwrapped.single_action_space  ## TODO: make a bounded gym Box out of it
    ...
    def __str__(self):
        ...
        return f"<{type(self).__name__}{self.env}>"  ## TODO: check what this does

    def __repr__(self):
        """Returns the string representation of the wrapper."""
        return str(self)  ## TODO: check what this does
    ...
    def class_name(cls) -> str:
        ...
        return cls.__name__  ## TODO: check what this does

    @property
    def unwrapped(self) -> RLTaskEnv:  ## TODO: Shouldn't be necessary, so remove
    ...
    def seed(self, seed: int | None = None) -> list[int | None]:  # noqa: D102
        return [self.unwrapped.seed(seed)] * self.unwrapped.num_envs  ## TODO: Change this, doesn't seem to exist in A1 so maybe just return a list of Nones, although the right thing to do is to set a seed in the environment and return that

    def reset(self) -> VecEnvObs:  # noqa: D102
        obs_dict, _ = self.env.reset()  ## Seems like it expects to return the observations and extras. Maybe in the extras we can return the critic observation. See this: https://isaac-orbit.github.io/orbit/source/api/orbit/omni.isaac.orbit.managers.html#omni.isaac.orbit.managers.ObservationManager
