...
from witp.rl.data.dataset import DatasetDict, _sample
...
class MemoryEfficientReplayBuffer(ReplayBuffer):
    ...
    def sample(
        ...
        if demo_size and demo_batch_size:
            demo_idx = self.np_random.integers(demo_size, size=demo_batch_size)
            indx[-len(demo_idx):] = demo_idx
